...
try{
    ...
    const DIDID = published.id();
    console.log("DIDID", DIDID.toString());
    const publishedd = await iotaResolution(DIDID.toString());
    ...
    const publishedA = await iotaResolution(process.env.DID_EXAMPLE);
    ...
    const unsignedVc = new Credential({
        ...
        issuer: publishedd.id(),
        credentialSubject: subject,
    });
        
    console.log("HI", publishedd.toJSON());
    console.log(publishedd.toString());
    console.log(storage.keyIdStorage());
    console.log(storage.keyStorage());
    console.log(fragment);

    // ****** FOR TESTING *******
    const [_did, _fragment] = publishedd.id().toString().split("#");
    const methodDigest = new MethodDigest(publishedd.methods()[0]);
    const _jwk_data = ((publishedd.methods()[0]).data().toJSON()).publicKeyJwk;
    const jwk_data = {
        ..._jwk_data,
    ...
    const credentialJwt = await publishedd.createCredentialJwt(
        // storage,
        issuerStorage,
        fragment,
        unsignedVc,
