import { CREATEPLAYER_MUTATION, UPDATESIGNEDIN_MUTATION, PLAYERID_QUERY ,PLAYER_QUERY} from "../graphql";
import { useMutation,useLazyQuery } from "@apollo/client";
import AES from 'crypto-js/aes';

export function useSignUp(){
    const [createPlayer] = useMutation(CREATEPLAYER_MUTATION);
    
    return async function(firstname, lastname, email, password){
        const username = `${firstname} ${lastname}`;
        try{
            const encryptPassword = AES.encrypt(password, 'webProgramming123').toString();
            const {data} = await createPlayer({variables:{name:username, password:encryptPassword, email}});
